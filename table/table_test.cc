...
TEST_P(BlockBasedTableTest, MseIndexTest) {
  ...
  for (int i = 0; i < kNumKeys; i++) {
    ...
    std::string key_str(RandomString(&rnd, kKeySize - 1) + "1");
    InternalKey k(key_str, 0, kTypeValue);
    std::string value_str(RandomString(&rnd, kValSize));
    c.Add(k.Encode().ToString(), value_str);
  }
