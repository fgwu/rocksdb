...
#include "table/block_mse_index.h"
...
inline uint32_t MseToBucket(const Slice& s, uint32_t num_buckets) {
  return rocksdb::Hash(s.data(), s.size(), kSeed) % num_buckets;
}

void BlockMseIndexBuilder::Add(const Slice& key,
                                  const uint32_t& restart_index) {
  uint32_t idx = MseToBucket(key, num_buckets_);
...
void BlockMseIndexBuilder::Finish(std::string& buffer) {
...
void BlockMseIndexBuilder::Reset() {
//  buckets_.clear();
  std::fill(buckets_.begin(), buckets_.end(), std::vector<uint32_t>());
  estimate_ = 0;
}

BlockMseIndex::BlockMseIndex(Slice block_content) {
...
void BlockMseIndex::Seek(Slice& key, std::vector<uint32_t>& bucket) const {
...
BlockMseIndexIterator* BlockMseIndex::NewIterator(
  ...
  return new BlockMseIndexIterator(data_ + bucket_off, limit, tag);
}

bool BlockMseIndexIterator::Valid() {
  return current_ < end_;
}

void BlockMseIndexIterator::Next() {
...
uint32_t BlockMseIndexIterator::Value() {
